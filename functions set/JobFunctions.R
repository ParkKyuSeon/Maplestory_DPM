...
EvanOptimization1 <- function(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, Specs, UnionBDRIGR, Cores=(detectCores(logical=F)-1), 
  ...
  BaseDeal <- EvanBoW(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, Specs, Cores, 
                      NotBuffCols, NotBuffColOption)
  ...
  while(ifelse(p<=2, 0, NewDeal[p-2])<=ifelse(p==1, 0, NewDeal[p-1]) & p<=nrow(PotList[[1]])) {
    ...
    NewSpecs$BDR <- Specs$BDR + PotList[[1]][p, 3]
    NewSpecs$IGR <- IGRCalc(c(Specs$IGR, PotList[[1]][p, 2]))
    NewDeal[p] <- sum(na.omit(EvanBoW(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, NewSpecs, Cores, 
                                      NotBuffCols, NotBuffColOption)))
    p <- p + 1
  }
  if(length(NewDeal)==2) {
  ...
  for(i in 1:nrow(AvailablePot)) {
    ...
    NewSpecs$BDR <- NewSpecs$BDR +  FinalList$BDR[1]
    NewSpecs$IGR <- IGRCalc(c(NewSpecs$IGR, FinalList$IGR[1]))
    NewDeal[1] <- sum(na.omit(EvanBoW(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, NewSpecs, Cores, 
                                      NotBuffCols, NotBuffColOption)))
    ...
    if(nrow(FinalList) > 1) {
      ...
      NewSpecs$BDR <- NewSpecs$BDR +  FinalList$BDR[2]
      NewSpecs$IGR <- IGRCalc(c(NewSpecs$IGR, FinalList$IGR[2]))
      NewDeal[2] <- sum(na.omit(EvanBoW(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, NewSpecs, Cores, 
                                        NotBuffCols, NotBuffColOption)))
      ...
      if(NewDeal[1] > NewDeal[2]) {
        ...
        while(NewDeal[p+1] <= NewDeal[p] & p + 2 <= nrow(FinalList)) {
          ...
          NewSpecs$BDR <- NewSpecs$BDR +  FinalList$BDR[p+2]
          NewSpecs$IGR <- IGRCalc(c(NewSpecs$IGR, FinalList$IGR[p+2]))
          NewDeal[p+2] <- sum(na.omit(EvanBoW(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, NewSpecs, Cores, 
                                              NotBuffCols, NotBuffColOption)))
          p <- p + 1
        }
        FinalCombination[i, 1] <- FinalList[length(NewDeal), 1]
