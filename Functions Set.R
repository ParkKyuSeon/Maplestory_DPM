...
Addoption <- function(ReqLv, AddOption=T, BossItem=T, Allstat=3, Single=3, Double1=3, Double2=0, Double3=0, ATK=0) {
  if(AddOption==T) {
    if(BossItem==T) {
    ...
    MainStat <- Single * stat + Double1 * statdouble + Double2 * statdouble + Double3 * statdouble
    SubStat1 <- Double1 * statdouble
    SubStat2 <- Double2 * statdouble
...
## Upgrade Function
### UpgradeOption : "1" : 30%, "2" : Miracle(ATK+2.5), "3" : AccScroll(ATK+2.5), 
### UpgradeOption : "4", PremiumAccScroll(ATK+4.2), "5", Iggdrasil(Stat+10, ReqLv>=120), "6" : Chaos(ATK+6)
Upgrade <- function(ReqLv, ItemType=c("Acc", "Armor", "Gloves"), Upgrade, GoldHammerAvailable=T, UpgradeOption, InitialATK) {
  ...
  Option2 <- ifelse(ItemType=="Acc", NA, 2.5)
  ...
  if(length(UpgradeOption)==Times) {
    ...
    if(is.na(MainStat)==F & is.na(ATK)==F) {
      Result <- as.data.frame(t(c(floor(ATK-InitialATK), 0, MainStat, 0, 0)))
...
HeartUpgrade <- function(Upgrade, UpgradeOption) {
  Times <- Upgrade + 1
  ATK <- 0
  MainStat <- 0
  SubStat1 <- 0
...
## StarForce Function
sfstat <- "https://raw.githubusercontent.com/ParkKyuSeon/Maplestory_DPM/master/starforcestat.csv"
sfstat <- read.csv(sfstat, header=T, row.names=1)
sfatk <- "https://raw.githubusercontent.com/ParkKyuSeon/Maplestory_DPM/master/starforceatk.csv"
sfatk <- read.csv(sfatk, header=T, row.names=1)
sfsup <- "https://raw.githubusercontent.com/ParkKyuSeon/Maplestory_DPM/master/superior.csv"
...
amaze <- "https://raw.githubusercontent.com/ParkKyuSeon/Maplestory_DPM/master/amazingstar.csv"
...
wsfstat <- "https://raw.githubusercontent.com/ParkKyuSeon/Maplestory_DPM/master/starforcestat.csv"
wsfstat <- read.csv(wsfstat, header=T, row.names=1)
wsfstat <- wsfstat[, 6:8]
wsfatk <- "https://raw.githubusercontent.com/ParkKyuSeon/Maplestory_DPM/master/weaponsfatk.csv"
...
PotentialList <- "https://raw.githubusercontent.com/ParkKyuSeon/Maplestory_DPM/master/potentiallist.csv"
