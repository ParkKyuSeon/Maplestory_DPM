...
info <- c(300, 12, 780, NA, NA, NA, NA, F)
info <- data.frame(AInfo, info)
colnames(info) <- c("option", "value")
...
option <- factor(c("IGR", "FDR"), levels=ASkill) 
value <- c(IGRCalc(c(50, ifelse(BattleMageCore[[1]][1, 2]>=40, 20, 0))), 2 * BattleMageCore[[1]][1, 2])
info <- c(300, 13, 780, NA, NA, NA, NA, F)
info <- data.frame(AInfo, info)
colnames(info) <- c("option", "value")
...
value <- c(20)
info <- c(500 + 20 * BattleMageCore[[2]][4, 2], 5, 0, 150, 0, NA, NA, F)
info <- data.frame(AInfo, info)
colnames(info) <- c("option", "value")
...
BattleMageCycle <- function(PreDealCycle, ATKFinal, BuffFinal, SummonedFinal, Spec, 
  ...
  Times100 <- c(0.5, 0.5, 0.5, 0.5, 0, 0.5, 0.5, 1, 0.5, 1, 4, 2.5, 1, 0.5)
...
BattleMageAddATK <- function(DealCycle, ATKFinal, SummonedFinal, Spec) {
  ...
  for(i in 1:nrow(DealCycle)) {
    if(sum(DealCycle$Skills[i]==c("FinishBlow", "FinishBlowUA", "FinishBlowUAMOD", "DarkLightning", "DarkLightningAL", "DarkLightningMark", "DarkLightningMarkDGFA",
                                  "DarkLightningMarkRemain", "DarkGenesis", "BattleKingBar1", "BattleKingBar2"))==1) {
...
DPM12338$BattleMage[1] <- sum(na.omit(BattleMageFinalDPMwithMax)) / (406380 / 60000)
DPM12338$BattleMage[2] <- sum(na.omit(BattleMageFinalDPM)) / (406380 / 60000) - sum(na.omit(BattleMageFinalDPMwithMax)) / (406380 / 60000)
...
BattleMageRR <- BattleMageDealData[19:199, ]
DPM12338$BattleMage[3] <- sum((BattleMageRR$Deal))

BattleMage40s <-  BattleMageDealData[19:475, ]
...
head(subset(BattleMageDealCycle2, BattleMageDealCycle2$Skills=="BlackMagicAltar"), 40)
subset(BattleMageDealCycle2, BattleMageDealCycle2$Skills=="GrimReeper")
subset(BattleMageDealCycle2, BattleMageDealCycle2$Skills=="DeathMOD")
subset(BattleMageDealCycle2, BattleMageDealCycle2$Skills=="Death")
subset(BattleMageDealCycle, BattleMageDealCycle$Skills=="SpiderInMirror1")
BattleMageDealCycle2[0:300, c(1, 2, 15)]
nrow(subset(BattleMageDealCycle2, BattleMageDealCycle2$Skills=="DarkLightningMark"))
nrow(subset(BattleMageDealCycle2, BattleMageDealCycle2$Skills=="DarkLightningMarkDGFA"))
nrow(subset(BattleMageDealCycle2, BattleMageDealCycle2$Skills=="DarkLightningMarkRemain"))
